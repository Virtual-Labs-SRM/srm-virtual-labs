...
export function BFSGraphCanvas({
  graph,
  bfsState,
  selectedNode,
  onNodeClick,
  ...
  const getNodeState = (nodeId: string) => {
    ...
    if (bfsState.currentNode === nodeId) return 'current';
    if (bfsState.visitedNodes.has(nodeId)) return 'visited';
    if (bfsState.queue.includes(nodeId)) return 'queued';
    return 'unvisited';
  };
    ...
    <div ref={containerRef} className="relative w-full h-full min-h-[500px]">
      ...
      <div className="absolute top-4 left-4 z-20 pointer-events-none">
        <Badge variant="secondary" className="backdrop-blur-md bg-background/50 border border-neutral-300 shadow-sm w-fit text-xs text-black">
          Steps: {bfsState.visitedNodes.size + (bfsState.currentNode ? 1 : 0)}
        </Badge>
      </div>
        ...
        {graph.nodes.map((node) => {
          ...
          return (
            ...
            >
                ...
                className={`transition-all duration-300 ${nodeState === 'current' ? 'fill-primary stroke-primary animate-pulse' :
                  nodeState === 'start' ? 'fill-primary stroke-primary stroke-[3]' :
                    nodeState === 'visited' ? 'fill-secondary stroke-secondary' :
                      nodeState === 'queued' ? 'fill-primary/30 stroke-primary stroke-2' :
              ...
              {/* Delete button indicator for remove mode */}
              {editMode === 'delete' && (
                <g onClick={(e) => {
                  e.stopPropagation();
